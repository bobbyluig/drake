#include "rgbd_to_lcm_point_cloud2.h"
...
RgbdToPointCloud2::RgbdToPointCloud2(const RgbdCamera2& camera) :
    camera_(camera) {
  ...
  point_cloud_output_index_ =
      DeclareAbstractOutputPort(
          &RgbdToPointCloud2::CalcPointCloudMessage).get_index();
}

const InputPortDescriptor<double>&
RgbdToPointCloud2::depth_image_input_port() const {
  return this->get_input_port(depth_image_input_index_);
}

const InputPortDescriptor<double>&
RgbdToPointCloud2::pose_vector_input_port() const {
  return this->get_input_port(pose_vector_input_index_);
}

const OutputPort<double>& RgbdToPointCloud2::point_cloud_output_port() const {
  return this->get_output_port(point_cloud_output_index_);
}

void RgbdToPointCloud2::CalcPointCloudMessage(
