#include "drake/examples/cosserat_rod/end_moment_case/cosserat_rod_plant.h"
...
// SpatialForce<T> F_W(Vector3<T>::Zero(),Vector3<T>(0.0, F0, 0.0));
//  //const Matrix3<T> R_WB = pc.get_X_WB(last_element_->get_node_index()).linear();
...
//  Fapplied_Bo_W_array[last_element_->get_node_index()] += R_WB * M_B;
  ...
  // Add a constant moment at the end link.
    const T M0 = 0.2;  // Torque in Nm
    SpatialForce<T> M_W(Vector3<T>(0.0, M0, 0.0), Vector3<T>::Zero());
    //const Matrix3<T> R_WB = pc.get_X_WB(last_element_->get_node_index()).linear();
