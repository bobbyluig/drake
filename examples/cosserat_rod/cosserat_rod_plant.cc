...
const RigidBody<T>& CosseratRodPlant<T>::AddElement(
  ...
  if (dimension_ == 2) {
    ...
    const SpaceXYZMobilizer<T>& mobilizer =
...
CosseratRodPlant<T>::DoMakeLeafContext() const {
...
void CosseratRodPlant<T>::SetBentState(
  ...
  for (int joint_index(0); joint_index < num_elements_; ++joint_index) {
    ...
    if (dimension_ == 3) {
      const multibody::SpaceXYZMobilizer<T>* mobilizer =
          dynamic_cast<const multibody::SpaceXYZMobilizer<T>*>(
              mobilizers_[joint_index]);
      mobilizer->set_angles(context, Vector3<T>(roll, 0.0, 0.0));
