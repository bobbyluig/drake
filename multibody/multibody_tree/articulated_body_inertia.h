...
class ArticulatedBodyInertia {
 public:
  ...
  /// Given `this` articulated body inertia `P_Q_E` for some articulated body
  /// computed about the origin of body Q, and expressed in frame E, this
  /// method computes:
  /// <pre>
  /// P_RQ_E = P_Q_E - P_Q_E H_RQ_E (H_RQ_Eᵀ P_Q_E H_RQ_E)⁻¹ H_RQ_Eᵀ P_Q_E
  /// </pre>
  /// where `H_RQ_E` is the geometric Jacobian relating bodies Q and R expressed
  /// in frame E.
  ///
  /// Physically, `P_RQ_E` is the effective articulated body inertia of some
  /// articulated body still about the origin of body Q, but measured from
  /// body R.
  ///
  /// For details see Section 6.2.4, Pages 104 - 105 of [Jain 2010].
  ///
  /// @param[in] H_RQ_E Across mobilizer geometric Jacobian that relates the
  ///                   body Q whose origin `this` articulated body inertia is
  ///                   computed about and body R, expressed in the same frame
  ///                   E `this` articulated body inertia is expressed in.
  /// @retval P_RQ_E This same articulated body inertia for some articulated
  ///         body but now measured from body R.
  ArticulatedBodyInertia<T>
  ProjectAcrossMobilizer(const MatrixUpTo6<T>& H_RQ_E) const {
    // Ensure that the geometric Jacobian has 6 rows.
    DRAKE_DEMAND(H_RQ_E.rows() == 6);

    // Get the number of columns of H_RQ_E.
    const int ncols = static_cast<const int>(H_RQ_E.cols());

    // Compute D_RQ_E, the articulated body hinge inertia according to Section
    // 6.2.2, Page 101 of [Jain 2010].
    MatrixUpTo6<T> D_RQ_E(ncols, ncols);
    D_RQ_E.template triangularView<Eigen::Lower>() =
        H_RQ_E.transpose() * *this * H_RQ_E;

    // Compute Dinv_RQ_E = D_RQ_E⁻¹. For matrices larger than 4x4, use LLT
    // decomposition instead since it is faster than an LU decomposition.
    MatrixUpTo6<T> Dinv_RQ_E;
    if (ncols > 4) {
      Dinv_RQ_E = D_RQ_E.template selfadjointView<Eigen::Lower>().llt().solve(
          MatrixUpTo6<T>::Identity(ncols, ncols));
    } else {
      Dinv_RQ_E = D_RQ_E.inverse();
    }

    // Ensure that D_RQ_E is not singular.
    // Singularity means that a non-physical geometric Jacobian was passed in
    // or that this articulated body inertia has some non-physical quantities
    // (such as zero moment of inertia along an axis which the geometric
    // Jacobian permits motion).
    DRAKE_DEMAND(Dinv_RQ_E.allFinite());

    // Compute g_RQ_E, the Kalman gain according to Section 6.2.2, Page 101 of
    // [Jain 2010].
    const MatrixUpTo6<T> g_RQ_E = *this * H_RQ_E * Dinv_RQ_E;

    // Perform projection as P_RQ_E = P_Q_E - g_RQ_E H_RQ_Eᵀ P_Q_E.
    ArticulatedBodyInertia<T> P_RQ_E;
    P_RQ_E.matrix_.template triangularView<Eigen::Lower>() =
        matrix_ - g_RQ_E * H_RQ_E.transpose() * *this;
